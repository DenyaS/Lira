...
void init_UART1(void)
{
  ...
  GPIO_Init(UART_GPIO_PORT, &gpio);
    ...
    // Настраиваем ногу DIR (PA2) как выход push-pull c основной функцией
  gpio.GPIO_Pin = DIR;
  gpio.GPIO_Speed = GPIO_Speed_50MHz;
...
void sendError_UARTx (uchar uart_number, uchar adr_dst, uchar cmd, uchar err)
{
    ...
    Error.Cmd = cmd;
	Error.Code = err;
    sendPacket_UARTx(uart_number, adr_dst, cmdERROR, (uchar *)&Error, sizeof(TError));
}
